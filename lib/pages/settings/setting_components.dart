...
/// 项目风格的可拖拽排序列表：长按拖拽（桌面 100ms / 移动 500ms），
/// 每项渲染为圆角卡片（surfaceContainerLow 底色 + outlineVariant 描边），
/// 配合右侧 drag_handle 图标使用。
class SettingReorderableList<T> extends StatefulWidget {
  final List<T> items;
  final Widget Function(T item) itemBuilder;
  final void Function(List<T> Function(List<T>) reorderFunc) onReorder;
  final double itemHeight;
  final Key Function(T item)? itemKey;

  const SettingReorderableList({
    super.key,
    required this.items,
  ...
  @override
  State<SettingReorderableList<T>> createState() =>
      _SettingReorderableListState<T>();
}

class _SettingReorderableListState<T> extends State<SettingReorderableList<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    return ReorderableBuilder<T>(
      ...
      children: widget.items.map((e) {
        return Padding(
          // ReorderableBuilder 要求每个直接子项带唯一 key
          key: widget.itemKey?.call(e) ?? ValueKey(e),
      ...
      builder: (children) {
        return GridView(
          ...
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            crossAxisCount: 1,
            mainAxisExtent: widget.itemHeight + 8,
          ),
          children: children,
        );
      },
