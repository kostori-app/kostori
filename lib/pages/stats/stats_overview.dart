...
class _ActivityHeatmap extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 自适应格子大小：完整铺进可用宽度（截图无需滚动）；
    // 短周期不超过 14px，不撑满整行；长周期自动缩小以容纳全部。
    return LayoutBuilder(
      builder: (context, constraints) {
        final maxWidth = constraints.maxWidth;
        const labelWidth = 16.0 + 4.0;
        var cellSize = (maxWidth - labelWidth) / weeks - gap;
        cellSize = cellSize.clamp(4.0, 14.0);
        while (weeks * (cellSize + gap) + labelWidth > maxWidth &&
            cellSize > 4.0) {
          cellSize -= 0.5;
        }
...
class _WatchPieChart extends StatelessWidget {
  const _WatchPieChart({required this.stats, required this.getRecords});
