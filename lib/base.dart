...
import 'package:kostori/tools/notification.dart';
...
class _Settings {
  ...
  /// 0/1 (detailed/brief)
  int get animeTileDisplayType =>
      int.parse(appdata.settings[44].split(',').first);

  set animeTileDisplayType(int v) {
  ...
  int get animesListDisplayType => int.parse(appdata.settings[25]);

  set animesListDisplayType(int value) {
    appdata.settings[25] = value.toString();
  }

  /// build-in comic sources
  bool isAnimeSourceEnabled(String key) {
  ...
  void setAnimeSourceEnabled(String key, bool enabled) {
    var index = builtInSources.indexOf(key);
    if (index == -1) {
      throw "Not Found";
    }
  ...
  // List<String> get explorePages {
  //   var pages = appdata.settings[77].split(',');
  //   // print("77行的值: ${appdata.settings[77]}");
  //   return pages;
  // }
