...
class CommentResponse {
  ...
  CommentResponse({required this.commentList, required this.total});

  factory CommentResponse.fromJson(Map<String, dynamic> json) {
    List? list = (json['list'] as List?) ?? (json['data'] as List?);
    List<CommentItem>? resCommentList = list
  ...
  factory CommentResponse.fromTemplate() {
    return CommentResponse(commentList: [], total: 0);
  }

  Map<String, dynamic> toJson() {
    return {'list': commentList, 'total': total};
  }
}
...
class EpisodeCommentResponse {
  ...
  EpisodeCommentResponse({required this.commentList});

  factory EpisodeCommentResponse.fromJson(List<dynamic> json) {
    List<EpisodeCommentItem>? resCommentList = (json as List?)
        ?.map((i) => EpisodeCommentItem.fromJson(i))
        .toList();
  ...
  factory EpisodeCommentResponse.fromTemplate() {
    return EpisodeCommentResponse(commentList: []);
  }
...
class CharacterCommentResponse {
  ...
  CharacterCommentResponse({required this.commentList});

  factory CharacterCommentResponse.fromJson(List<dynamic> json) {
    List<CharacterCommentItem>? resCommentList = (json as List?)
        ?.map((i) => CharacterCommentItem.fromJson(i))
        .toList();
  ...
  factory CharacterCommentResponse.fromTemplate() {
    return CharacterCommentResponse(commentList: []);
  }
