...
class SliverGridDelegateWithAnimes extends SliverGridDelegate {
  SliverGridDelegateWithAnimes([this.useBriefMode = false, this.scale]);
  ...
  @override
  SliverGridLayout getLayout(SliverConstraints constraints) {
    if (appdata.settings['animeDisplayMode'] == 'brief' || useBriefMode) {
      return getBriefModeLayout(
        constraints,
        scale ?? (appdata.settings['animeTileScale'] as num).toDouble(),
      );
    } else {
