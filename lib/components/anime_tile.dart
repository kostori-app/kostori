...
class AnimeTileMenuOption {
  final String title;
  final IconData icon;
  final void Function(String? animeId) onTap;

  const AnimeTileMenuOption(this.title, this.icon, this.onTap);
}

abstract class AnimeTile extends StatelessWidget {
      ...
      children: [
        ...
        Center(
          child: FilledButton(
            child: const Text("确认"),
            onPressed: () {
              LocalFavoritesManager().addAnime(folder!, favoriteItem!);
              context.pop();
            },
          ),
        ),
  ...
  Widget build(BuildContext context) {
    ...
    var isFavorite = appdata.settings[72] == '1'
        ? LocalFavoritesManager().isExist(animeID!)
        : false;
...
class NormalAnimeTile extends AnimeTile {
  const NormalAnimeTile(
...
class AnimeTilePlaceholder extends StatelessWidget {
  const AnimeTilePlaceholder({super.key, this.type = 'full'});
...
class CustomAnimeTile extends AnimeTile {
  const CustomAnimeTile(this.anime, {super.key, this.addonMenuOptions});

  final CustomAnime anime;

  @override
  String get description => anime.description;

  @override
  Widget get image => AnimatedImage(
        image: StreamImageProvider(
            () =>
                ImageManager().getCustomThumbnail(anime.cover, anime.sourceKey),
            anime.id),
        fit: BoxFit.cover,
        width: double.infinity,
        height: double.infinity,
  ...
  @override
  String get subTitle => anime.subTitle;

  @override
  String get title => anime.title;

  @override
  FavoriteItem? get favoriteItem => FavoriteItem.custom(anime);
  ...
  get read => () async {
        ...
        var animeSource = AnimeSource.find(anime.sourceKey)!;
        var res = await animeSource.loadAnimeInfo!(anime.id);
        if (cancel) return;
        dialog.close();
        if (res.error) {
...
Widget buildAnimeTile(BuildContext context, BaseAnime item, String sourceKey,
    {List<AnimeTileMenuOption>? addonMenuOptions}) {
  var source = AnimeSource.find(sourceKey);
  if (source == null) {
    throw "Anime Source $sourceKey Not Found";
  }
  ...
  //   }
  // }
  if (source.animeTileBuilderOverride != null) {
    return source.animeTileBuilderOverride!(
      context,
      item,
      addonMenuOptions,
    );
...
//         Button.filled(onPressed: onSubmit, child: Text("提交")),
//         const SizedBox(height: 16),
//       ],
//     );
//
